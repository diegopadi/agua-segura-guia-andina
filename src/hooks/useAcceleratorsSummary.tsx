  ...
  const generateSummary = async () => {
    ...
    try {
      ...
      if (!acceleratorsData || acceleratorsData.length === 0) {
        toast({
          title: "Sin datos",
          description: "No se encontraron datos de los aceleradores 1, 2 y 3. Completa primero estas etapas.",
          variant: "destructive"
        });
        return;
      }
      ...
      const { data, error } = await supabase.functions.invoke('generate-accelerators-summary', {
        body: {
          acceleratorsData,
          profileData: {
      ...
      if (data.success) {
        const summary: HallazgosClave = {
          ...
          region: `${profile.ie_region || ""}${profile.ie_province ? ', ' + profile.ie_province : ''}`
        };
