...
export function useCNPIEProject(tipoProyecto: '2A' | '2B' | '2C') {
  const { user } = useAuth();
  ...
  const saveAcceleratorData = async (etapa: number, acelerador: number, data: any) => {
    if (!proyecto) return;
    ...
    try {
      ...
      setProyecto(prev => prev ? { ...prev, datos_aceleradores: updatedDatos } : null);
