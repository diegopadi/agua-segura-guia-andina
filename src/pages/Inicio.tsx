          ...
          <Dialog open={isVideoOpen} onOpenChange={setIsVideoOpen}>
            ...
            <DialogContent className="max-w-4xl">
              ...
              <div className="aspect-video">
                <iframe src="https://www.youtube.com/embed/dQw4w9WgXcQ" // Placeholder URL - cambiar cuando tengas el video real
              title="Video de Bienvenida" className="w-full h-full rounded-lg" allowFullScreen />
              </div>
            </DialogContent>
          </Dialog>
        </div>
            ...
            <div className="mt-4">
              <div className="text-sm text-muted-foreground mb-2">
                {loading ? <div className="flex items-center gap-2">
                    <Loader2 className="w-3 h-3 animate-spin" />
                    Cargando...
                  </div> : `Progreso: ${progress.accelerator2}%`}
              </div>
              <div className="w-full bg-muted rounded-full h-2">
                <div className="bg-secondary h-2 rounded-full transition-all duration-500 ease-out" style={{
                width: `${progress.accelerator2}%`
              }}></div>
              </div>
            </div>
          </CardContent>
            ...
            <div className="mt-4">
              <div className="text-sm text-muted-foreground mb-2">
                {loading ? <div className="flex items-center gap-2">
                    <Loader2 className="w-3 h-3 animate-spin" />
                    Cargando...
                  </div> : `Progreso: ${progress.accelerator3}%`}
              </div>
              <div className="w-full bg-muted rounded-full h-2">
                <div className="bg-accent h-2 rounded-full transition-all duration-500 ease-out" style={{
                width: `${progress.accelerator3}%`
              }}></div>
              </div>
            </div>
          </CardContent>
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <Button variant="outline" className="gap-2" onClick={() => window.location.href = 'mailto:luis.alban@helvetas.org?subject=Consulta sobre Mi Cole con Agua Segura'}>
              <Mail className="w-4 h-4" />
              Contactar a Helvetas Perú
            </Button>
            <Button variant="outline" className="gap-2" onClick={() => window.location.href = 'mailto:luva@equilibria.lat?subject=Consulta sobre Mi Cole con Agua Segura'}>
              <Mail className="w-4 h-4" />
