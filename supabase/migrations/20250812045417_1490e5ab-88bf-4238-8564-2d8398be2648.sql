...
create policy if not exists "Authenticated can read app configs"
  on public.app_configs for select
  using (auth.role() = 'authenticated');

create policy if not exists "Authenticated can insert app configs"
  on public.app_configs for insert
  with check (auth.role() = 'authenticated');

create policy if not exists "Authenticated can update app configs"
